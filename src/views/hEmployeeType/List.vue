<template>
  <CRow>
        ...
        <template v-slot:default="{ hide }">
          <form>
            <div class="row mb-2">
              <div class="col-md-12">
                <label>Code 
                  <font-awesome-icon :style="{ color: 'darkorange' }" v-c-tooltip.hover.click="'Required, unique'" icon="info-circle"/>
                </label>
                <input :class="{ 'is-invalid' : errorMessages!=null && errorMessages.employee_type_code!=undefined}" v-model="form.employee_type_code" type="text" class="form-control">
                <div class="invalid-feedback" v-if="errorMessages!=null && errorMessages.employee_type_code!=undefined">
            ...
            <div class="row mb-2">
              <div class="col-md-12">
                <label>Employee Type <font-awesome-icon :style="{ color: 'darkorange' }" v-c-tooltip.hover.click="'Required'" icon="info-circle"/></label>
                <input :class="{ 'is-invalid' : errorMessages!=null && errorMessages.employee_type_name!=undefined}" v-model="form.employee_type_name" type="text" class="form-control">
                <div class="invalid-feedback" v-if="errorMessages!=null && errorMessages.employee_type_name!=undefined">
                    {{errorMessages.employee_type_name[0]}}
                </div>
              </div>
            </div>
            <div class="row mb-2">
    ...
    <div>
        ...
        <div class="row mb-2">
            <div class="col-md-12">
                <TextViewer :text="form.employee_type_code" :label="'Code'"/>
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-md-12">
                <TextViewer :text="form.employee_type_name" :label="'Employee Type'"/>
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-md-12">
                <TextViewer :text="form.description" :label="'Description'"/>
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-md-6">
                <TextViewer :text="form.user_i.name" :label="'User Input'"/>
            </div>
            <div class="col-md-6">
                <TextViewer :text="form.user_e.name" :label="'User Edit'"/>
            </div>
        </div>
...
export default {
  ...
  data: () => {
    return {
      urlData : url + 'employeetype/filter',
  ...
  computed : {
    ...
    column(){
      return [
          {'column' : 'employee_type_code', 'name' : 'Code', 'format' : this.getBadge},
          {'column' : 'employee_type_name', 'name' : 'Employee Type'},
  ...
  methods :{
    ...
    cleanForm(){
        this.form.id = '';
        this.form.employee_type_code = '';
        this.form.employee_type_name = '';
    ...
    deleteAct(data){
      if (confirm("Are you sure delete selected data?")==true){
        this.status = "delete";
        axios.delete(''+url+'employeetype/'+data.id+'',{
    ...
    saveAct(){ 
      ...
      if (this.status=="save"){
        //save
        axios.post(''+url+'employeetype',this.form,{
          headers : this.headerAccess
        }).then((res)=>{
            this.isLoading = false;
            this.processResponse(res,this.status);
        ...
        axios.post(''+url+'employeetype/'+this.form.id+'/update',this.form,{
          headers : this.headerAccess
        }).then((res)=>{
            this.isLoading = false;
            this.processResponse(res,this.status);
